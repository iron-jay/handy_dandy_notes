...
If (!($OSD)){
    Write-Output "--------------------------------------" 
    Write-Output "Starting PNPUtil Driver Install" 
    ...
    if (Test-Path $SourcePath){ 
        ...
        $Output = "$LogPath\PnpUtilDriversOutput.txt" 
        $Drivers = Get-ChildItem $SourcePath -Recurse -Filter "*.inf" 
        $Total = $Drivers.Count 
        $Counter = 1 
        Write-Output "$Total drivers found" 
        Foreach ($Driver in $Drivers){
            $DriverFullPath = $Driver.FullName 
            Write-Output "Start-Process pnputil.exe -ArgumentList ""/add-driver $DriverFullPath /install"" -PassThru -NoNewWindow -RedirectStandardOutput $Output" 
            Start-Process pnputil.exe -ArgumentList "/add-driver $DriverFullPath /install" -PassThru -NoNewWindow 
            Show-TSActionProgress -Message "Installing $Driver" -Step $Counter -MaxStep $Total -ErrorAction SilentlyContinue 
            $Counter += 1 
            }  

        Write-Output "PNPUtil Step Complete" 
        Write-Output "See PNPUtil log for more Details: $Output" 
        }
    else{
